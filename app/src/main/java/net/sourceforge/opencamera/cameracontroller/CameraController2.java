...
import net.sourceforge.opencamera.R; // <-- gemini_81dlp
...
import android.view.View; //gemini_81dlp
...
public class CameraController2 extends CameraController {
    ...
    // gemini_81dlp: Helper method to append hidden Camera ID "52" if it's not present
// gemini_81dlp: Dynamic list with strict validity checks (No static cache)
    private String[] getExtendedCameraIdList(android.hardware.camera2.CameraManager manager) throws android.hardware.camera2.CameraAccessException {
        String[] orig = manager.getCameraIdList();
        java.util.List<String> allCameraIds = new java.util.ArrayList<>(java.util.Arrays.asList(orig));
        ...
        if (context != null) {
            try {
                ...
                String manualIdsStr = sharedPreferences.getString("preference_manual_camera_ids", "");
                
                if (manualIdsStr != null && !manualIdsStr.trim().isEmpty()) {
                    String[] manualIds = manualIdsStr.split(",");
                    for (String manualId : manualIds) {
                        String cleanId = manualId.trim();
                        if (!cleanId.isEmpty() && !allCameraIds.contains(cleanId)) {
                            try {
                                // Hard check: Does this ID actually exist in hardware?
                                if (manager.getCameraCharacteristics(cleanId) != null) {
                                    allCameraIds.add(cleanId);
                                }
                            } catch (Throwable e) {
                                // Invalid ID (e.g., "5") throws an exception. Skip it silently.
                            }
                        }
                    }
                }
    ...
    public CameraController2(Context context, int cameraId, String cameraIdSPhysical, Map<String, CameraFeaturesCache> camera_features_caches, final ErrorCallback preview_error_cb, final ErrorCallback camera_error_cb) throws CameraControllerException {
        ...
        try {
            ...
            this.cameraIdS = getExtendedCameraIdList(manager)[cameraId]; // gemini_81dlp
            if( MyDebug.LOG )
...
//      Frame duration is in nanoseconds.  Using min to be safe.
    ...
    public void updatePreviewTexture() {
        ...
        if( texture != null ) {
            if( preview_width == 0 || preview_height == 0 ) {
                ...
                // gemini_81dlp: Sync right eye surface buffer resolution
                try {
                    Activity activity = (Activity) context;
                    View rightContainer = activity.findViewById(net.sourceforge.opencamera.R.id.preview_right);
                    if (rightContainer != null) {
                        TextureView rightEyeView = (TextureView) rightContainer.findViewWithTag("right_eye_texture");
                        if (rightEyeView != null && rightEyeView.getSurfaceTexture() != null) {
                            rightEyeView.getSurfaceTexture().setDefaultBufferSize(preview_width, preview_height);
                        }
                    }
    ...
    private void createCaptureSession(boolean wait_until_started, Runnable runnable, Runnable on_failed, final MediaRecorder video_recorder, boolean want_photo_video_recording) throws CameraControllerException {
        ...
        // gemini_81dlp: Pre-fetch the right eye surface for SBS stereoscopic VR
        Activity activity = (Activity) context;
        View rightContainer = activity.findViewById(net.sourceforge.opencamera.R.id.preview_right);
        TextureView rightEyeView = (rightContainer != null) 
            ? (TextureView) rightContainer.findViewWithTag("right_eye_texture") 
            : null;

        if (rightEyeView != null && rightEyeView.getSurfaceTexture() != null && preview_width > 0 && preview_height > 0) {
            rightEyeView.getSurfaceTexture().setDefaultBufferSize(preview_width, preview_height);
        }

        final Surface right_preview_surface = (rightEyeView != null && rightEyeView.isAvailable()) 
            ? new Surface(rightEyeView.getSurfaceTexture()) 
            : null;
        // gemini_81dlp
            ...
            class MyStateCallback extends CameraCaptureSession.StateCallback {
                ...
                void onConfigured(@NonNull CameraCaptureSession session, @NonNull CameraExtensionSession eSession) {
                    ...
                    synchronized( background_camera_lock ) {
                        ...
                        // gemini_81dlp: Route the preview frame feed to the right eye
                        if (right_preview_surface != null && right_preview_surface.isValid()) {
                            previewBuilder.addTarget(right_preview_surface);
                        }// gemini_81dlp
...
Surface preview_surface;
            ...
            synchronized( background_camera_lock ) {
                ...
                if( video_recorder != null ) {
                    if( supports_photo_video_recording && !want_video_high_speed && want_photo_video_recording ) {
                        surfacesList.add(preview_surface);
                        surfacesList.add(video_recorder_surface);
                    ...
                    surfacesList.add(preview_surface);
                    surfacesList.add(imageReader.getSurface());
                    surfacesList.add(imageReaderRaw.getSurface());
                }
                else {
                    surfacesList.add(preview_surface);
                    surfacesList.add(imageReader.getSurface());
                ...
                // gemini_81dlp: Register the right eye surface with the Android Camera HAL
                if (right_preview_surface != null && right_preview_surface.isValid()) {
                    surfacesList.add(right_preview_surface);
                }
                ...
                if( MyDebug.LOG ) {
                    ...
                    Log.d(TAG, "surfaces: " + surfacesList);
                }
            } // gemini_81dlp
            final List<Surface> surfaces = surfacesList;            if( MyDebug.LOG ) {
                if( video_recorder == null ) {
                    if( imageReaderRaw != null ) {
