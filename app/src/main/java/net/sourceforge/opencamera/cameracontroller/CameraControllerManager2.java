...
// gemini_81dlp: Dynamic list with strict validity checks (No static cache)
    ...
    public int getNumberOfCameras() {
        android.hardware.camera2.CameraManager manager = (android.hardware.camera2.CameraManager)context.getSystemService(android.content.Context.CAMERA_SERVICE);
        try {
            // Force the UI to count the manually injected cameras too
            return getExtendedCameraIdList(manager).length;
        }
        catch(Throwable e) {
            MyDebug.logStackTrace(TAG, "exception trying to get number of cameras", e);
            return 0;
        }
    }
    /*
    ...
    public int getNumberOfCameras() {
        ...
        try {
            return getExtendedCameraIdList(manager).length; // gemini_81dlp
        }
        catch(Throwable e) {
    ...
    public CameraController.Facing getFacing(int cameraId) {
        CameraManager manager = (CameraManager)context.getSystemService(Context.CAMERA_SERVICE);
        try {
            String[] idList = getExtendedCameraIdList(manager);
            if (idList == null || idList.length == 0) {
                return CameraController.Facing.FACING_BACK;
            }
            // Strict index bounds clamping to prevent crashes if indices drift
            if (cameraId < 0 || cameraId >= idList.length) {
                cameraId = 0; 
            }
            String cameraIdS = idList[cameraId]; 
    ...
    public String getDescription(Context context, int cameraId) {
        ...
        try {
            String cameraIdS = getExtendedCameraIdList(manager)[cameraId]; // gemini_81dlp
